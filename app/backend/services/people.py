...
class PeopleService:
    ...
    async def create(self, data: Dict[str, Any], user_id: Optional[str] = None) -> Optional[People]:
        """Create a new people"""
        try:
            if user_id:
    ...
    async def check_ownership(self, obj_id: int, user_id: str) -> bool:
        ...
        try:
            ...
            logger.error(f"Error checking ownership for people {obj_id}: {str(e)}")
            return False

    async def get_by_id(self, obj_id: int, user_id: Optional[str] = None) -> Optional[People]:
        """Get people by ID (user can only see their own records)"""
        try:
            query = select(People).where(People.id == obj_id)
            if user_id:
    ...
    async def get_list(
        ...
        """Get paginated list of peoples (user can only see their own records)"""
        try:
            query = select(People)
            count_query = select(func.count(People.id))
            
            if user_id:
                query = query.where(People.user_id == user_id)
                count_query = count_query.where(People.user_id == user_id)
    ...
    async def update(self, obj_id: int, update_data: Dict[str, Any], user_id: Optional[str] = None) -> Optional[People]:
        """Update people (requires ownership)"""
        try:
            obj = await self.get_by_id(obj_id, user_id=user_id)
            if not obj:
    ...
    async def delete(self, obj_id: int, user_id: Optional[str] = None) -> bool:
        """Delete people (requires ownership)"""
        try:
            obj = await self.get_by_id(obj_id, user_id=user_id)
            if not obj:
